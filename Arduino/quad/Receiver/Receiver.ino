...
PID myPID1(&Input1, &Output1, &Setpoint1, 2,2,0, REVERSE);    //Pitch BWD
PID myPID2(&Input1, &Output2, &Setpoint1, 2,2,0, DIRECT);     //Pitch FWD
PID myPID3(&Input2, &Output3, &Setpoint2, 2,2,0, DIRECT);     //Roll LEFT
PID myPID4(&Input2, &Output4, &Setpoint2, 2,2,0, REVERSE);    //Roll RIGHT
...
void setup()
{
   // SetSampleTime(100);
    ...
    //=======================================
      radio.begin();
      radio.openReadingPipe (1, pipe_Rx); // Open one of the 6 channels of reception
...
void loop()
{
  if( radio.available())
  {
    ...
    {
            getposition();
            done = radio.read( package_Rx, sizeof(package_Rx));     
            throttle_val = package_Rx[0];  // 0 = Zero      190 = MAX
            bank_val     = package_Rx[1];
            pitch_val    = package_Rx[2];
...
// ------------------------------------ ( 1 ALL THROTTLE) ------------------------------------
            ...
            {   
              ...
              m[3] = constrain(throttle_val - 10, 0, motormax); 
              motor1.write(m[0]);
              motor2.write(m[1]);
              motor3.write(m[2]);
              motor4.write(m[3]);  
...
// ------------------------------------ ( 2 BANK RIGHT) ------------------------------------
            ...
            {     
              ...
              m[3] = constrain((throttle_val - 10 - bank_right), 0, motormax);  
              motor1.write(m[0]);
              motor2.write(m[1]);
              motor3.write(m[2]);
              motor4.write(m[3]);    
...
// ------------------------------------ ( 3 BANK LEFT) ------------------------------------
            ...
            {     
              ...
              m[3] = (constrain((throttle_val - 10 + bank_left), 0, motormax));   
              motor1.write(m[0]);
              motor2.write(m[1]);
              motor3.write(m[2]);
              motor4.write(m[3]);    
...
// ------------------------------------ ( 4 PITCH BWD) ------------------------------------
            ...
            {     
              ...
              m[3] = (constrain((throttle_val - 10 + pitch_bwd), 0, motormax));  
              motor1.write(m[0]);
              motor2.write(m[1]);
              motor3.write(m[2]);
              motor4.write(m[3]);    
...
// ------------------------------------ ( 5 PITCH FWD) ------------------------------------
            if( pitch_val >= high  &&   bank_val >= low && bank_val <= high)
            {
              ...
              m[3] = (constrain((throttle_val - 10 - pitch_fwd), 0, motormax)); 
              motor1.write(m[0]);
              motor2.write(m[1]);
              motor3.write(m[2]);
              motor4.write(m[3]);    
...
// ------------------------------------ ( 6 BANK LEFT && PITCH BWD) ------------------------------------
            if( bank_val <= low  &&  pitch_val <= low)
            {
              ...
              m[3] = (constrain((throttle_val - 10 + bank_left + pitch_bwd), 0, motormax));
              motor1.write(m[0]);
              motor2.write(m[1]);
              motor3.write(m[2]);
              motor4.write(m[3]);    
...
//=========================== 1 ======*/  // Pitch BWD 
    if( ypr[1] >= (yprd[0] + low2) &&     ypr[2] >= (yprd[1] - low2)  &&  ypr[2] <= (yprd[1] + low2))    
    {
      ...
      m[3] = constrain(throttle_val - 10 + pitch_bwd - Output1, 0, motormax); 
      motor1.write(m[0]);
      motor2.write(m[1]);
      motor3.write(m[2]);
      motor4.write(m[3]);   
...
//=========================== 2 ======*/  // Pitch FWD  
    if( ypr[1] <= (yprd[0] - low2) &&     ypr[2] >= (yprd[1] - low2)  &&  ypr[2] <= (yprd[1] + low2) )  
    {
      ...
      m[3] = constrain(throttle_val - 10 - pitch_fwd + Output2, 0, motormax); 
      motor1.write(m[0]);
      motor2.write(m[1]);
      motor3.write(m[2]);
      motor4.write(m[3]); 
...
//===================== ====== 3 ======*/  // Roll LEFT
    if( ypr[2] <= (yprd[1] - low2) &&     ypr[1] >= (yprd[0] - low2)  &&  ypr[1] <= (yprd[0] + low2) )   
    {
      ...
      m[3] = constrain(throttle_val - 10 + bank_left - Output3, 0, motormax); 
      motor1.write(m[0]);
      motor2.write(m[1]);
      motor3.write(m[2]);
      motor4.write(m[3]); 
...
//=========================== 4 ======*/  // Roll RIGHT  
    if( ypr[2] >= (yprd[1] + low2) &&     ypr[1] >= (yprd[0] - low2)  &&  ypr[1] <= (yprd[0] + low2) )
    {
      ...
      m[3] = constrain(throttle_val - 10 - bank_right + Output4, 0, motormax); 
      motor1.write(m[0]);
      motor2.write(m[1]);
      motor3.write(m[2]);
      motor4.write(m[3]); 
...
//================================= 5 ======*/  // Picth Bwd && Roll Right
    if( ypr[1] >= (yprd[0] + low2) &&  ypr[2] >= (yprd[1] + low2) )
    {
      ...
      m[3] = constrain(throttle_val - 10 - bank_right + pitch_fwd - Output1 + Output4, 0, motormax); 
      motor1.write(m[0]);
      motor2.write(m[1]);
      motor3.write(m[2]);
      motor4.write(m[3]);
...
//=========================== 6 ======*/  // Picth Bwd && Roll Left  
    if( ypr[1] >= (yprd[0] + low2) && ypr[2] <= (yprd[1] - low2)) 
    {
      ...
      m[3] = constrain(throttle_val - 10 + bank_left + pitch_bwd - Output1 - Output3, 0, motormax);         
      motor1.write(m[0]);
      motor2.write(m[1]);
      motor3.write(m[2]);
      motor4.write(m[3]); 
...
//=========================== 7 ======*/  // Picth Fwd && Roll Left  
    if( ypr[1] <= (yprd[0] - low2) && ypr[2] <= (yprd[1] - low2) )
    {
      ...
      m[3] = constrain(throttle_val - 10 + bank_left - pitch_fwd + Output2 - Output3, 0, motormax); 
      motor1.write(m[0]);
      motor2.write(m[1]);
      motor3.write(m[2]);
      motor4.write(m[3]); 
