...
PID myPID1(&Input1, &Output, &Setpoint1, 2,2,0, REVERSE);    //Pitch BWD
PID myPID2(&Input2, &Output, &Setpoint2, 2,2,0, DIRECT);     //Pitch FWD
PID myPID3(&Input3, &Output, &Setpoint3, 2,2,0, DIRECT);     //Roll LEFT
PID myPID4(&Input4, &Output, &Setpoint4, 2,2,0, REVERSE);    //Roll RIGHT
...
void setup()
{
    ...
    myPID2.SetOutputLimits(0, 20);
    myPID3.SetOutputLimits(0, 20);
    myPID4.SetOutputLimits(0, 20);
 ...
 // SetSampleTime(100);
  changech2 = pulseIn(3, HIGH, 25000);
   ...
   // {
           ...
           Setpoint2= Setpoint1 = 3.63;    // Pitch
            Setpoint3=Setpoint4 = -1.13;    // Roll   
...
// ------------------------------------ ( 1 ALL THROTTLE) ------------------------------------
            ...
            {   
              ...
              m[1] = constrain(throttle_val+5, 0, motormax);
...
// ------------------------------------ ( 2 BANK RIGHT) ------------------------------------
            ...
            {     
              ...
              m[1] = constrain((throttle_val+5 - bank_right), 0, motormax);
...
// ------------------------------------ ( 3 BANK LEFT) ------------------------------------
            ...
            {     
              ...
              m[1] = (constrain((throttle_val+5 + bank_left), 0, motormax));
...
// ------------------------------------ ( 4 PITCH BWD) ------------------------------------
            ...
            {     
              ...
              m[1] = (constrain((throttle_val+5 + pitch_bwd), 0, motormax));
...
// ------------------------------------ ( 5 PITCH FWD) ------------------------------------
            if( pitch_val >= high  &&   bank_val >= low && bank_val <= high)
            {
              ...
              m[1] = (constrain((throttle_val+5 - pitch_fwd), 0, motormax));
...
// ------------------------------------ ( 6 BANK LEFT && PITCH BWD) ------------------------------------
            if( bank_val <= low  &&  pitch_val <= low)
            {
              ...
              m[1] = (constrain((throttle_val+5 + bank_left + pitch_bwd), 0, motormax));
...
// ------------------------------------ ( 7 BANK LEFT && PITCH FWD) ------------------------------------
            ...
            {     
              ...
              m[1] = (constrain((throttle_val+5 + bank_left - pitch_fwd), 0, motormax));
...
// ------------------------------------ ( 8 BANK RIGHT && PITCH FWD) ------------------------------------
            ...
            {     
              ...
              m[1] = (constrain((throttle_val+5 - bank_right - pitch_fwd), 0, motormax));
...
// ------------------------------------ ( 9 BANK RIGHT && PITCH BWD) ------------------------------------
            if( bank_val >= high  &&  pitch_val <= low)
            {
              ...
              m[1] = (constrain((throttle_val+5 - bank_right + pitch_bwd), 0, motormax));
...
//=========================== 1 ======*/  // Pitch BWD 
    if( ypr[1] >= (yprd[0] + low2) &&     ypr[2] >= (yprd[1] - low2)  &&  ypr[2] <= (yprd[1] + low2))    
    {
      ...
      m[1] = constrain(throttle_val+5 + pitch_bwd - Output1, 0, motormax);
...
//=========================== 2 ======*/  // Pitch FWD  
    if( ypr[1] <= (yprd[0] - low2) &&     ypr[2] >= (yprd[1] - low2)  &&  ypr[2] <= (yprd[1] + low2) )  
    {
     ...
     // myPID2.Compute();    
      ...
      m[1] = constrain(throttle_val+5 - pitch_fwd + Output2, 0, motormax);
...
//===================== ====== 3 ======*/  // Roll LEFT
    ...
    Output1=0;
     // myPID3.Compute();     
      ...
      m[1] = constrain(throttle_val+5 + bank_left - Output3, 0, motormax);
...
//=========================== 4 ======*/  // Roll RIGHT  
    if( ypr[2] >= (yprd[1] + low2) &&     ypr[1] >= (yprd[0] - low2)  &&  ypr[1] <= (yprd[0] + low2) )
    {
     ...
     Output1= 0; 
      ...
      m[1] = constrain(throttle_val+5 - bank_right + Output4, 0, motormax);
...
//================================= 5 ======*/  // Picth Bwd && Roll Right
    ...
    Output2=0;
      ...
      m[0] = constrain(throttle_val + bank_right - pitch_bwd + Output1 - Output4, 0, motormax);
      m[1] = constrain(throttle_val+5 - bank_right - pitch_bwd+  Output1 + Output4, 0, motormax);
      m[2] = constrain(throttle_val - bank_right + pitch_bwd -Output1 + Output4, 0, motormax);
      m[3] = constrain(throttle_val + bank_right + pitch_bwd - Output1 - Output4, 0, motormax); 
...
//=========================== 6 ======*/  // Picth Bwd && Roll Left  
    ...
    Output2=0;
     ...
     // myPID3.Compute();          
      ...
      m[1] = constrain(throttle_val+5 + bank_left + pitch_bwd - Output1 - Output3, 0, motormax);
...
//=========================== 7 ======*/  // Picth Fwd && Roll Left  
   ...
   //   myPID3.Compute();        
      ...
      m[1] = constrain(throttle_val+5 + bank_left - pitch_fwd + Output2 - Output3, 0, motormax);
...
//=========================== 8 ======*/  // Picth Fwd && Roll Right  
  ...
  //    myPID2.Compute();       
    //  myPID4.Compute();        
      ...
      m[1] = constrain(throttle_val+5 - bank_right - pitch_fwd + Output2 + Output4, 0, motormax);
      m[2] = constrain(throttle_val - bank_right + pitch_fwd - Output2 + Output4, 0, motormax);
      m[3] = constrain(throttle_val + bank_right + pitch_fwd - Output2 - Output4, 0, motormax); 
              up1.write(m[0]);
              up2.write(m[1]);
              down1.write(m[2]);
              down2.write(m[3]);  
