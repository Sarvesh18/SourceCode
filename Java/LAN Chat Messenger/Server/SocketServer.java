...
public class SocketServer implements Runnable {
    ...
    public int clientCount = 0, port = 8090;
    public ServerFrame ui;
    public Database db;
	...
	if (msg.content.equals(".bye")){
			...
			else if(msg.type.equals("share_req")){
                if(msg.recipient.equals("All")){
                    clients[findClient(ID)].send(new Message("message", "SERVER", "Uploading to 'All' forbidden", msg.sender));
                }
                else{
					String IP = findUserThread(msg.sender).socket.getInetAddress().getHostAddress();
                    findUserThread(msg.recipient).send(new Message("share_req", IP, msg.content, msg.recipient));
                }
            }
            else if(msg.type.equals("share_res")){
                if(!msg.content.equals("NO")){
                    String IP = findUserThread(msg.sender).socket.getInetAddress().getHostAddress();
                    findUserThread(msg.recipient).send(new Message("share_res", IP, msg.content, msg.recipient));
                }
                else{
                    findUserThread(msg.recipient).send(new Message("share_res", msg.sender, msg.content, msg.recipient));
                }
            }
